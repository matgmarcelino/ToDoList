...
function createSidebarComponents(projects) {
    const brandContainer = (() => {
        ...
        brand.classList.add('sidebar-brand-container');

        const accent = document.createElement('span');
        accent.classList.add('sidebar-brand-accent');
        accent.textContent = 'ker';

        div.append(brand.append(accent));
        return div;
    })();
...
function createProjectDiv(project) {
    ...
    const tasks = (() => {
        ...
        projects.tasks.forEach((t) => tasksInner.append(createTaskDiv(t)));
    })();
}
...
function createTrashIcon({ size = 24, color = '#bf4545', className = '' } = {}) {
    const div = document.createElement('div');
